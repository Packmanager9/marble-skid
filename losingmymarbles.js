...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Circle {
        ...
        frictiveMove() {
            ...
            this.xmom += this.xjerk
            this.ymom += this.yjerk
            this.x += this.xmom
            this.y += this.ymom
            this.xmom *= this.friction
            this.ymom *= this.friction
            this.xjerk *=(this.friction*this.friction*this.friction)
            this.yjerk *=  (this.friction*this.friction*this.friction)
        }
        frictiveunMove() {
            if (this.reflect == 1) {
                if (this.x + this.radius > canvas.width) {
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Marble{
        constructor(){
            this.body = new Circle(100, (normalmarbles.length*100)+100, 5+(normalmarbles.length*2.5), getRandomColor(), 0 , 0, 1-(((5+(normalmarbles.length*5.5))/1000)))
            console.log(this)
        }
        draw(){
            this.body.draw()
    ...
    function main() {
        ...
        // // game code goes here
        for(let t = 0;t<normalmarbles.length;t++){
            normalmarbles[t].move()
            normalmarbles[t].draw()
        }
    }
})
